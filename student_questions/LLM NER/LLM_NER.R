...
txtFiles <- list.files(path = '~/Desktop/ICPSR/student_questions/LLM NER',
                       pattern = '*.txt',
                       full.names = T)
...
write.csv(documentLocations, 
          '~/Desktop/ICPSR/student_questions/LLM NER/documentLocations.csv', 
          row.names = F)
...
write.csv(consolidatedInfo, 
          '~/Desktop/ICPSR/student_questions/LLM NER/consolidatedInfo.csv', 
          row.names = F)
...
for(i in 1:nrow(consolidatedInfo)){
  ...
  tmpLLMResponse <- rawLLM[idx]
  tmpLLMResponse <- unlist(lapply(strsplit(tmpLLMResponse, ':'),tail, 1))
  #if()
  response <- data.frame(rawResponse = rawLLM,
                         perp = tmpLLMResponse[1],
                         type = tmpLLMResponse[2],
                         victim = tmpLLMResponse[3],
                         row.names = NULL)
}
